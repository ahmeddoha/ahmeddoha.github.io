(function () {
  ...
  function renderModules() {
    ...
    panel.innerHTML = `
      <div class="cycle-question"><small>The course question</small><h3>${module.question}</h3><p>${module.business}</p><span class="verdict">${module.verdict}</span></div>
      <div class="cycle-study"><small>Classes in this stage · ${module.classes.length}</small><ol class="module-case-list">${module.classes.map(classTitle => `<li>${classTitle}</li>`).join("")}</ol><p><b>Conceptual and analytical focus:</b> ${module.focus}</p><b>${module.evidence}</b></div>
      <div class="cycle-methods"><small>Frameworks you will use</small><div>${module.methods.map(method => `<span>${method}</span>`).join("")}</div></div>
      <div class="cycle-output"><small>What you produce</small><p>${module.make}</p></div>
      <div class="cycle-result"><small>How the project advances</small><p>${module.carry}</p></div>`;
  ...
  function renderClasses() {
    const filters = [["all", "All 13"], ["technology", "Technologies"], ["application", "Applications"], ["business", "Business"], ["defense", "Project defense"]];
  ...
  function readChecks() {
    try { return JSON.parse(localStorage.getItem("busi4331-student-ready") || "[]"); } catch (_) { return []; }
  }
  function writeChecks(values) {
    try { localStorage.setItem("busi4331-student-ready", JSON.stringify(values)); } catch (_) {}
  }
