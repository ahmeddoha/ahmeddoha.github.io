(function () {
  ...
  function renderModules() {
    ...
    panel.innerHTML = `
      <div class="cycle-question"><small>The strategic question</small><h3>${module.question}</h3><p>${module.business}</p><span class="verdict">${module.verdict}</span></div>
      <div class="cycle-study"><small>Module cases · ${module.cases.length}</small><ol class="module-case-list">${module.cases.map(caseTitle => `<li>${caseTitle}</li>`).join("")}</ol>${module.relatedSessions ? `<div class="related-sessions"><small>Related project session</small>${module.relatedSessions.map(session => `<span>${session}</span>`).join("")}</div>` : ""}<p><b>Analytical focus:</b> ${module.focus}</p><b>${module.evidence}</b></div>
      ...
      <div class="cycle-result"><small>What carries into the platform project</small><p>${module.carry}</p></div>`;
  ...
  function renderClasses() {
    const filters = [["all", "All 13"], ["prepare", "Foundation"], ["analyze", "Case analysis"], ["studio", "Project studio"], ["defend", "Defence"]];
  ...
  function readChecks() {
    try { return JSON.parse(localStorage.getItem("busi4607-student-ready") || "[]"); } catch (_) { return []; }
  }
  function writeChecks(values) {
    try { localStorage.setItem("busi4607-student-ready", JSON.stringify(values)); } catch (_) {}
  }
