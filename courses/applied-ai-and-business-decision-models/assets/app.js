(function () {
  ...
  function renderClasses() {
    const filters = [["all", "All 13"], ["learn", "Learn + frame"], ["execute", "Execute + interpret"], ["challenge", "Challenge + decide"], ["present", "Present + defend"]];
    ...
    function classCard(item) {
      ...
      const label = cycle ? `Cycle ${cycle.number} · ${cycle.name}` : item.cycle === "presentations" ? "Term Paper Presentations" : "Course foundation";
    ...
    const rows = [{
      key: "foundation", number: "Course foundation", name: "Data → decisions", range: "Class 1", color: "#e91c24",
      items: classes.filter(item => item.cycle === "foundation")
    }, ...D.cycles.map(cycle => ({
      key: cycle.key,
      ...
      key: "capstone", number: "Term Paper Presentations", name: "Present + defend", range: "Classes 12–13", color: "#191919",
      items: classes.filter(item => item.cycle === "presentations")
    ...
    $("#classGrid").innerHTML = rows.map(row => `
      <section class="semester-cycle-row ${row.key === "capstone" ? "capstone-row" : ""}" style="--cycle:${row.color}">
        ...
        <div class="cycle-class-grid ${activeFilter !== "all" ? "filtered" : ""} ${row.items.length === 1 && row.key !== "foundation" ? "solo" : ""}">${row.items.map(classCard).join("")}</div>
  ...
  function readChecks() {
    try { return JSON.parse(localStorage.getItem("busi4301-student-ready") || "[]"); } catch (_) { return []; }
  }
  function writeChecks(values) {
    try { localStorage.setItem("busi4301-student-ready", JSON.stringify(values)); } catch (_) {}
  }
